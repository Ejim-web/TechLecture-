...
function updateAuthUI() {
  ...
  if (loginBtn) {
    loginBtn.textContent = isAuthModeLogin ? "Login / Signup" : "Sign Up";
  }
}
  ...
  courses.forEach(course => {
    ...
    card.innerHTML = `
      ...
      <div class="card-actions">
        <button class="primary btn-sm" onclick="window.openCourse('${course.id}')">
          <i class="fas fa-play"></i> Start Learning
        </button>
        <button class="btn btn-sm" onclick="window.bookmarkCourse('${course.id}')">
...
window.searchCourses = function(value) {
...
window.bookmarkCourse = async function(id) {
  if (!currentUser) {
...
const loginBtn = $("loginBtn");
if (loginBtn) {
  loginBtn.onclick = window.openAuthModal;
}
...
if (searchBtn) {
  searchBtn.onclick = () => {
    ...
    if (input) window.searchCourses(input.value);
  };
}
...
if (searchInput) {
  searchInput.addEventListener("keyup", (e) => {
    if (e.key === "Enter") {
      window.searchCourses(e.target.value);
    }
  });
}
...
// Google Login
window.googleLogin = loginGoogle;

// GitHub Login
window.githubLogin = loginGithub;

// Admin - Add Course
const addCourseBtn = $("addCourse");
if (addCourseBtn) {
  addCourseBtn.onclick = window.addCourse;
}

// Certificate Verification
const verifyBtn = $("verifyCertificate");
if (verifyBtn) {
  verifyBtn.onclick = window.verifyCertificate;
}
...
if (sendAI) {
  sendAI.onclick = async () => {
    ...
    const reply = await window.askAI(message);
